...
namespace TicTocGuardians.Scripts.Game.Player
{
    ...
    {
        private List<List<Action>> _actions = new();
        ...
        public void RecordStart(Player target)
        {
            ...
            var beaver = target as Beaver;
            if (beaver != null) CreateBeaverRecordStream(beaver);
        }
        ...
        public void CreateBeaverRecordStream(Beaver beaver)
        {
            GlobalInputBinder.CreateGetKeyDownStream(KeyCode.B)
                ...
                .Subscribe(_ => { _currentActionBuffer.Add(new Action(Action.State.Special)); }).AddTo(gameObject);
        ...
        public void CreateMovementRecordStream(Player target)
        {
            ...
            GlobalInputBinder.CreateGetAxisStreamOptimize("Horizontal").TakeWhile(_ => _isRecord)
                .Subscribe(v => _currentActionBuffer.Add(new Action(Action.State.MoveX, v))).AddTo(target.gameObject);
            GlobalInputBinder.CreateGetAxisStreamOptimize("Vertical").TakeWhile(_ => _isRecord)
                .Subscribe(v => _currentActionBuffer.Add(new Action(Action.State.MoveZ, v))).AddTo(target.gameObject);
            GlobalInputBinder.CreateGetKeyDownStream(KeyCode.Space).TakeWhile(_ => _isRecord)
                .Subscribe(_ => _currentActionBuffer.Add(new Action(Action.State.Jump))).AddTo(target.gameObject);
