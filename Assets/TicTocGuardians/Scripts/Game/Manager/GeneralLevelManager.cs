...
namespace TicTocGuardians.Scripts.Game.Manager
{
    public class GeneralLevelManager : LevelManager
    {
        ...
        [Header("ป๓ลย")] [SerializeField] private Phase currentState;
        ...
        {
            _phaseSubject.Where(x => x == Phase.Ready).Subscribe(_ => { OnReadyPhaseActive(); });

            _phaseSubject.Where(x => x == Phase.Ordering).Subscribe(_ => { OnOrderingPhaseActive(); });

            _phaseSubject.Where(x => x == Phase.Play1).Subscribe(_ => { OnPlay1PhaseActive(); });

            _phaseSubject.Where(x => x == Phase.Play2).Subscribe(_ => { OnPlay2PhaseActive(); });

            _phaseSubject.Where(x => x == Phase.Play3).Subscribe(_ => { OnPlay3PhaseActive(); });
        ...
        public void OnPlay1PhaseActive()
        {
            ...
            var player = SpawnPlayer(_playerOrder[_currentPlayPhaseIndex],
                currentSpawnPointIndices[_currentPlayPhaseIndex]);
        ...
        public void OnPlay3PhaseActive()
        {
            ...
            SpawnPlayer(_playerOrder[_currentPlayPhaseIndex], currentSpawnPointIndices[_currentPlayPhaseIndex]);
            PlayPhaseStart(playerController);
            foreach (var instance in playerInstances) CreateDimensionCheckStream(instance);
        }
        ...
        public override void ActiveLevel(PlayerController controller)
        {
            ...
            foreach (var clone in _currentClones) clone.CreateMovementStream();
        ...
        public PlayerClone CreateClone(PlayerCloneData data, int index)
        {
            var instance = Instantiate(clonePrefabs[(int)data.type - 1], origin);
            instance.transform.position = spawnPoints[index].transform.position + Vector3.up * 3;
        ...
        {
            for (var i = 0; i < _currentPlayPhaseIndex; i++) CreateClone(_cloneData[i], currentSpawnPointIndices[i]);
        }

        public void DestroyAllPlayer()
        {
            foreach (var player in playerInstances) Destroy(player.gameObject);
