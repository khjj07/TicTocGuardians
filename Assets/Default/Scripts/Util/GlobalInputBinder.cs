...
namespace Default.Scripts.Util
{
    public class GlobalInputBinder : Singleton<GlobalInputBinder>
    {
        ...
        {
            return Instance.UpdateAsObservable().Where(_ => Input.GetMouseButton(btn)).Select(_ => Input.GetMouseButton(btn));
        }
        public static IObservable<bool> CreateGetMouseButtonDownStream(int btn)
        {
            return Instance.UpdateAsObservable().Where(_ => Input.GetMouseButtonDown(btn)).Select(_ => Input.GetMouseButtonDown(btn));
        }
        public static IObservable<bool> CreateGetMouseButtonUpStream(int btn)
        {
            return Instance.UpdateAsObservable().Where(_ => Input.GetMouseButtonUp(btn)).Select(_ => Input.GetMouseButtonUp(btn));
        }
